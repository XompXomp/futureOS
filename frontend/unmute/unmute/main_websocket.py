...
async def receive_loop(
    ...
    while True:
        ...
        if isinstance(message, ora.InputAudioBufferAppend):
            ...
            # Add support for direct text input from the frontend yes
            if hasattr(message, 'type') and message.type == 'conversation.item.input_text':
                user_text = getattr(message, 'text', None)
                patient_profile = getattr(message, 'patientProfile', None)
                if patient_profile is not None:
                    handler.update_patient_profile(patient_profile)
                    # Force a rebuild of the system prompt with the new profile
                    instructions = handler.chatbot.get_instructions()
                    if instructions:
                        handler.chatbot.set_instructions(instructions, handler.patient_profile)
                if user_text:
                    logger.info(f"Received text input: {user_text}")
                    # Add the user message to the chat history
                    await handler.chatbot.add_chat_message_delta(user_text, "user")
                    # Generate a response as if it was from STT
                    await handler._generate_response()
            elif hasattr(message, 'type') and message.type == 'llama.extra_info':
                handler.llama_extra_info = getattr(message, 'extra_info', None)
                logger.info(f"[DEBUG] Received llama.extra_info: {handler.llama_extra_info}")
                # Only trigger a response if the LLM is not already in progress
                if not handler.llm_in_progress:
                    await handler._generate_response()
            else:
    ...
    while True:
        ...
        try:
            ...
            if emitted_by_handler is None:
                ...
                # If this is response.audio.done, finalize the writer
                if to_emit.type == "response.audio.done":
                    opus_writer = None
                    awaiting_audio_done = False
            else:
                _sr, audio = emitted_by_handler
                audio = audio_to_float32(audio)
                # If not already started, start a new writer for this reply
                if opus_writer is None:
                    opus_writer = sphn.OpusStreamWriter(SAMPLE_RATE)
                    awaiting_audio_done = True
                ...
                if opus_bytes:
                    ...
                    continue
        # Log every event sent to the frontend
        logger.info(f"[DEBUG] Emitting to frontend: {getattr(to_emit, 'type', str(type(to_emit)))}")
...
async def get_messages():
    ...
    if latest_handler is not None:
        return {"messages": latest_handler.chatbot.chat_history}
    else:
        return {"messages": []}
