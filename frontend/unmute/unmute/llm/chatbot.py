...
class Chatbot:
    ...
    def set_instructions(self, instructions: Instructions, patient_profile: dict | None = None):
        # If a patient profile is provided, inject it into the system prompt.
        system_prompt = instructions.make_system_prompt()
        if patient_profile:
            profile_str = f"\n# PATIENT PROFILE\n{json.dumps(patient_profile, indent=2)}\n"
            if '\n# BASICS' in system_prompt:
                # Insert after BASICS section
                parts = system_prompt.split('\n# BASICS', 1)
                system_prompt = parts[0] + '\n# BASICS' + profile_str + (parts[1] if len(parts) > 1 else '')
            else:
