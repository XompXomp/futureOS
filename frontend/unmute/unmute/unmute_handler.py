...
class UnmuteHandler(AsyncStreamHandler):
    def __init__(self) -> None:
        ...
        self.patient_profile = None  # Store patient profile from frontend
    ...
    async def _generate_response_task(self):
        ...
        # Inject extra info from Llama if present
        if hasattr(self, 'llama_extra_info') and self.llama_extra_info:
            logger.info(f"[DEBUG] Injecting llama_extra_info into LLM prompt: {self.llama_extra_info}")
            messages.append({
                "role": "system",
                "content": f"Extra info from Llama: {self.llama_extra_info}"
            })
            self.llama_extra_info = None  # Clear after use
            logger.info("[DEBUG] Cleared llama_extra_info after injection")
        else:
            logger.info("[DEBUG] No llama_extra_info to inject into LLM prompt")
        ...
        try:
            ...
            # If extraInfo is waiting, trigger another response
            if hasattr(self, 'llama_extra_info') and self.llama_extra_info:
                logger.info("[DEBUG] Triggering follow-up response for queued extraInfo")
        ...
        try:
            async for data in stt:
                if isinstance(data, STTMarkerMessage):
                    ...
                    continue
                logger.info(f"[DEBUG] Emitting transcription event: {data.text}")
                await self.output_queue.put(
                ...
                logger.info(f"[DEBUG] Emitted transcription event: {data.text}")
    ...
    async def update_session(self, session: ora.SessionConfig):
        if session.instructions:
            self.chatbot.set_instructions(session.instructions, self.patient_profile)
    ...
    def update_patient_profile(self, profile: dict | None):
